 ...
 * @author ylo019
 */
public class YankeeSwapStandardStochastic extends StochasticAlgorithm {
    
    protected double k = 0.0;
    
    public YankeeSwapStandardStochastic(double newK) {
        k = newK;
    }
    ...
    protected iProbabilityMatrix solve(Permutation priority, PreferenceProfile input, int agents, int objects) {
        ...
        for (Permutation p : presentOrder) {
            ...
            try {
                ...
                throw new RuntimeException("YSSS: solve(): matching generated is invalid");
            }
        }
        return output;
    }
    ...
    private int[] solveFixedPresentOrder(Permutation priority, Permutation presentOrder, PreferenceProfile input, int agents) {
        ...
        boolean[][] alreadyStolen = new boolean[agents][agents]; //marker for already stolen object
        ...
        while (pp.hasNext()) {
            ...
            while (true) {
                ...
                while (ip.hasNext(actingAgent)) { //scan the actingAgent's preference to use rule to decide whether to open
                    ...
                    if (desiredItem == 0) { //no target yet
                        if (obj[itemInvestigating - 1] == 0) {
                            ...
                            //opened
                            if (!alreadyStolen[actingAgent - 1][itemInvestigating - 1]) {
                ...
                if (rule(desiredItem, itemBetterThanBest, itemWorseThanBest)) {
                    ...
                    alreadyStolen[temp - 1][desiredItem - 1] = true; //last owner cannot steal it back

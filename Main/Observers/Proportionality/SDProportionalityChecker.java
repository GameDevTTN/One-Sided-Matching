 ...
 * @author ylo019
 */
public class SDProportionalityChecker extends iResultsCollator {
    ...
    protected void onEndPreference() {
        ...
        PostBox.broadcast(MessageType.PROCESS, new Pair<>("SDProportional Allocations", new ArrayList<>(isSDProportional)));
        clear();
    }
    ...
    protected boolean isProportional(iProbabilityMatrix matrix, PreferenceProfile pp, double factor) {
        ...
        for (int i = 1; i <= pp.size(); i++) {
            if (!pp.doesSDPrefer(i, matrix.read(i), proportional)) {
                return false;
            }
        }
        return true;
